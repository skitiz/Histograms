...
void broadcastdata(long int* intervals, int* min, int* max, unsigned long long int* size, unsigned long long int* local_size, MPI_Comm comm);
...
int main(int argc, char* argv[])
{
		...
		if(my_rank == 0)
		{
			...
			local_buffer = (local_size * sizeof(int));

		}
		void broadcastdata(&intervals, &min, &max, &size, &local_size, comm);
...
int* max_min(char* filename, unsigned long long int* size, int *max, int *min, unsigned long long int* amount)
{
		FILE *fp;
		*max = INT_MIN;
		*min = INT_MAX;
